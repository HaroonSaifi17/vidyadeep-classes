...
function StatItem({ value, suffix, label, started, delay }: StatItemProps) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 max-w-45 text-fine leading-relaxed text-cream-muted">
        {label}
      </p>
    </div>
  );
...
export function TrackRecord() {
  ...
  return (
    <section
      id="record"
      ref={ref}
      className="scroll-mt-24 bg-ink py-16 text-cream sm:py-24"
    >
      <Container>
        <Reveal>
          <div className="max-w-2xl">
            ...
            <p className="mt-5 max-w-lg text-body leading-relaxed text-band-muted">
              Vidyadeep is a new campus — but the teaching behind it isn’t. Our
              mentors have guided students through boards and entrance exams for
              years, with extensive one-on-one mentoring along the way.
            </p>
          </div>
        </Reveal>
            ...
            <div className="grid items-center gap-8 lg:grid-cols-2">
              <div>
                ...
                <h3 className="font-display text-h3">
                  Celebrating every student milestone.
                </h3>
                <p className="mt-3 max-w-md text-body leading-relaxed text-band-muted">
                  From scholarship awards to monthly test toppers — we celebrate
                  progress at every step. Visit our campus to see our full wall
                  of honor and past student achievements.
                </p>
                <div className="mt-6 flex items-center gap-3">
                  <span className="h-2.5 w-2.5 rounded-full bg-gold animate-pulse" />
                  <span className="text-fine font-semibold text-gold">
                    Real Campus Award Ceremonies
                  </span>
                </div>
              </div>
              ...
              <div className="relative overflow-hidden rounded-xl border border-white/20 bg-white/5 p-2 shadow-2xl">
                ...
                <div className="absolute bottom-4 left-4 right-4 rounded-lg bg-ink/90 p-3 text-center backdrop-blur-md">
                  ...
                  <p className="text-fine text-cream-muted">
                    Vidyadeep Mentorship Heritage
                  </p>
                </div>
              </div>
            </div>
